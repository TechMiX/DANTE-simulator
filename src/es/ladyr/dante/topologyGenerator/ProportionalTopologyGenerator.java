...
public class ProportionalTopologyGenerator implements TopologyGenerator {
    ...
    public void generateTopology() {
        ...
        for(int nodeIndex = 0; nodeIndex < nodesArray.length; nodeIndex++)
            allNodesCapacitiesSum += nodesArray[nodeIndex].getCapacity()*normalizationFactor;
        ...
        for(int nodeIndex = 0; nodeIndex < nodesArray.length; nodeIndex++){
            ...
            long partialCapsSum = (long)(allNodesCapacitiesSum - node.getCapacity()*normalizationFactor);
            for(int connCounter = 0; connCounter < newNeighbors.length; connCounter++){                
                DanteNode candidate = chooseNode(partialCapsSum, nodesCopy);                
                partialCapsSum -= candidate.getCapacity()*normalizationFactor;
    ...
    protected DanteNode chooseNode(long allCapacitiesSum, SortedArrayList nodes){
        ...
        for(int nodeIndex = 0; nodeIndex < nodesArray.length; nodeIndex++){
            DanteNode node = nodesArray[nodeIndex];
            sumOfCaps += node.getCapacity()*normalizationFactor;
